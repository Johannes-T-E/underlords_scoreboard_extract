...
class NetWorthDigitDetector:
    ...
    def find_digits_by_sliding(self, networth_region, confidence_threshold=0.98):
...
class NetWorthExtractor:
    ...
    def __init__(self, debug=False):
        self.networth_detector = NetWorthDigitDetector()
        self.debug = debug
    ...
    def extract_networth_value(self, image, row_y, networth_column_x):
        ...
        if self.debug:
            print(f"NetWorth templates loaded: {len(self.networth_detector.digit_templates)} templates")
            if self.networth_detector.digit_templates:
                print(f"Available networth digits: {list(self.networth_detector.digit_templates.keys())}")
        
        # Find all digit matches using sliding window
        digit_matches = self.networth_detector.find_digits_by_sliding(networth_region)
        
        if digit_matches:
            # Reconstruct number from digit matches
            number_result = self.networth_detector.reconstruct_number_from_matches(digit_matches)
