...
class HealthExtractor:
    ...
    def __init__(self, debug=False):
        self.health_detector = HealthDigitDetector()
        self.debug = debug
    ...
    def extract_health_value(self, image, row_y, health_column_x):
        ...
        if self.debug:
            print(f"Health templates loaded: {len(self.health_detector.digit_templates)} templates")
            if self.health_detector.digit_templates:
                print(f"Available health digits: {list(self.health_detector.digit_templates.keys())}")
        
        # Find all digit matches using sliding window
        digit_matches = self.health_detector.find_digits_by_sliding(health_region)
        
        if digit_matches:
            # Reconstruct number from digit matches
            number_result = self.health_detector.reconstruct_number_from_matches(digit_matches)
