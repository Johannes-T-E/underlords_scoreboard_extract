...
class DigitDetector:
    ...
    def find_digits_by_sliding(self, number_region, confidence_threshold=0.98):
...
class PlayerExtractor:
    ...
    def extract_player_level_region(self, image, row_y):
        ...
        level_region = image[info_y_start:info_y_end, PLAYER_INFO_X_START:PLAYER_INFO_X_START + 12]
        
        return level_region
    
    def extract_player_gold_region(self, image, row_y):
        """Extract the player gold region (after first 12px) from a row."""
        info_y_start = row_y + PLAYER_INFO_Y_START
        info_y_end = info_y_start + PLAYER_INFO_HEIGHT
    ...
    def extract_player_name(self, image, row_y):
        """Extract player name using template matching + OCR fallback."""
        ...
        try:
            ...
            if player_name and len(player_name) > 1:  # Valid name found
                # Create new template
                new_template = self.template_manager.add_new_player(name_region, player_name)
                
                if self.debug:
                    print(f"OCR found new player: {player_name} (created template {new_template['template_id']})")
                ...
                return {"player_name": "", "template_id": None, "method": "ocr_failed"}
    ...
    def extract_player_level(self, image, row_y):
        ...
        # Try sliding digit detection first
        if self.debug:
            print(f"Digit templates loaded: {len(self.digit_detector.digit_templates)} templates")
            if self.digit_detector.digit_templates:
                print(f"Available digits: {list(self.digit_detector.digit_templates.keys())}")
    ...
    def _parse_gold(self, text):
        ...
        # Look for patterns like "Gold 32", "32g", etc.
        gold = 0
        
        gold_matches = re.findall(r'(\d+)', text)
        for match in gold_matches:
            potential_gold = int(match)
            if potential_gold > 0 and potential_gold <= 200:  # Reasonable gold range
                gold = potential_gold
                break
    ...
    def extract_all_player_data(self, image, row_y, row_number):
        ...
        name_result = self.extract_player_name(image, row_y)
...
# Usage example
def extract_players_from_scoreboard(image, header_positions, config):
    ...
    for row_num, row_y in enumerate(row_boundaries):
        ...
        if player_data["playerName"] or player_data["playerLevel"] > 0 or player_data["playerGold"] > 0:
            players_data.append(player_data)
        elif config.debug:
            print(f"Skipping row {row_num} - no meaningful player data")
